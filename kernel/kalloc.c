...
// lock and freelist for each CPU
struct {
...
kalloc(void)
{
  ...
  // try to alloc page for current cpu
  acquire(&kmem.lock[cpu_id]);
  ...
  // if previous attempt wasn't successful, try to steal page from other cpu
  if(!r){
    for(int i = 0; i < NCPU; i++) {
      acquire(&kmem.lock[i]);
      r = kmem.freelist[i];
